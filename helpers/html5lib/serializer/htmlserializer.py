...
from helpers.html5lib.constants import voidElements, booleanAttributes, spaceCharacters
from helpers.html5lib.constants import rcdataElements
...
try:
    ...
    from helpers.html5lib.constants import entities
...
class HTMLSerializer(object):
    ...
    def serialize(self, treewalker, encoding=None):
        ...
        if encoding and self.inject_meta_charset:
            from helpers.html5lib.filters.inject_meta_charset import Filter
            treewalker = Filter(treewalker, encoding)
        # XXX: WhitespaceFilter should be used before OptionalTagFilter
        # for maximum efficiently of this latter filter
        if self.strip_whitespace:
            from helpers.html5lib.filters.whitespace import Filter
            treewalker = Filter(treewalker)
        if self.sanitize:
            from helpers.html5lib.filters.sanitizer import Filter
            treewalker = Filter(treewalker)
        if self.omit_optional_tags:
            from helpers.html5lib.filters.optionaltags import Filter
            treewalker = Filter(treewalker)
